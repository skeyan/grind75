...
import { postorderTraversal, TreeNode } from "./solution.js";
...
describe("Binary Tree Postorder Traversal", () => {
  it("returns empty array for empty tree", () => {
    assert.deepStrictEqual(postorderTraversal(null), []);
  });

  it("returns single value for one node", () => {
    const root = new TreeNode(1);
    assert.deepStrictEqual(postorderTraversal(root), [1]);
  });

  it("postorder for root with right subtree and nested left (LeetCode heap shape)", () => {
  ...
  it("handles LeetCode example [3,9,20,null,null,15,7]", () => {
    ...
    assert.deepStrictEqual(postorderTraversal(root), [9, 15, 7, 20, 3]);
  });

  it("handles left-skewed tree", () => {
    const root = fromArray([1, 2, null, 3, null, null, null, 4]);
    assert.deepStrictEqual(postorderTraversal(root), [4, 3, 2, 1]);
  });
});
