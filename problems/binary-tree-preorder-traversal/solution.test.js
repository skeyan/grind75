...
import { preorderTraversal, TreeNode } from "./solution.js";
...
describe("Binary Tree Preorder Traversal", () => {
  ...
  it("returns single value for one node", () => {
    ...
    assert.deepStrictEqual(preorderTraversal(root), [1]);
  });

  it("preorder for root → right → left subtree (LeetCode shape needs heap indices)", () => {
    const root = fromArray([1, null, 2, null, null, 3]);
    assert.deepStrictEqual(preorderTraversal(root), [1, 2, 3]);
  });

  it("handles LeetCode example [3,9,20,null,null,15,7]", () => {
    const root = fromArray([3, 9, 20, null, null, 15, 7]);
    assert.deepStrictEqual(preorderTraversal(root), [3, 9, 20, 15, 7]);
  });

  it("handles left-skewed tree", () => {
    const root = fromArray([1, 2, null, 3, null, null, null, 4]);
    assert.deepStrictEqual(preorderTraversal(root), [1, 2, 3, 4]);
  });
});
